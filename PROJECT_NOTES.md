...
## Latest Modifications (2026-04-27)

- Auth has been implemented and connected to MySQL (`park_guide_database`) for both `user_page` and `admin_page`.
- Backend auth registration now supports role-based account creation:
  - `role = admin` maps to `role_id = 1`
  - `role = guide` maps to `role_id = 2` and creates `guide_profiles` row
- New auth UI components were added:
  - `admin_page/src/components/AuthPortal.jsx`
  - `admin_page/src/components/AuthPortal.css`
  - `user_page/src/components/AuthPortal.jsx`
  - `user_page/src/components/AuthPortal.css`
- `admin_page` now routes through an auth wrapper (`admin_page/src/App.jsx`) before showing the dashboard.
- `user_page` now uses an auth gate and logout handling in `user_page/src/App.jsx`.
- Forgot-password and reset-password UI flows are wired to backend endpoints.
- Dev server API proxy was added for both portals:
  - `admin_page/vite.config.js`
  - `user_page/vite.config.js`
- A user-page navigation bug was fixed where login/logout transitions sometimes needed a browser refresh:
  - Root cause: auth return happened before all hooks were declared in `user_page/src/App.jsx`.
  - Fix: moved auth-gate return below hook declarations so hook order stays stable.
