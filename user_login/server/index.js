...
const configuredAuthCorsOrigins = !process.env.CORS_ORIGIN || process.env.CORS_ORIGIN === '*'
  ? []
  : process.env.CORS_ORIGIN.split(',').map((origin) => origin.trim()).filter(Boolean)

const authCorsOriginSetting = !process.env.CORS_ORIGIN || process.env.CORS_ORIGIN === '*'
  ? true
  : [...new Set([...configuredAuthCorsOrigins, ...defaultAuthCorsOrigins])]

const authCorsOptions = {
  origin: authCorsOriginSetting,
  credentials: true,
...
const startServer = async () => {
  ...
  const authHost = process.env.AUTH_API_HOST || process.env.HOST || '0.0.0.0'

  app.listen(port, authHost, async () => {
    ...
    console.log(`Server running on http://${authHost === '0.0.0.0' ? 'localhost' : authHost}:${port} (bound ${authHost})`)
