...
const CHART_COLORS = ["#6bdc45", "#ff7a1a", "#ffd23f", "#874500"];
      ...
      try {
        ...
        if (isMounted) {
          setState({
            ...
            data: {
              stats: [
                { label: "Active Guides", value: statsJson.activeGuides, change: "+3.1%", icon: <People sx={{ color: "#874500" }} />, progress: 70 },
                { label: "Total Modules", value: statsJson.totalModules, change: "+2.4%", icon: <BookIcon sx={{ color: "#ff7a1a" }} />, progress: 100 },
                { label: "Certifications Issued", value: statsJson.certificationsIssued, change: "+6.2%", icon: <CheckCircle sx={{ color: "#6bdc45" }} />, progress: 60 },
                { label: "Active Incidents", value: statsJson.activeIncidents, change: "-1.8%", icon: <NotificationsIcon sx={{ color: "#ffd23f" }} />, progress: 40 },
              ],
...
function Dashboard() {
  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{ mb: 4 }}>
        <Typography variant="h5" sx={{ fontWeight: 700, mb: 3, color: "var(--forest, #3a2a16)", pb: 2, borderBottom: "2px solid var(--line)" }}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 2 }}>
          {[
            ...
            <Paper key={item.label} sx={{ p: 2, borderRadius: 3, textAlign: "center", transition: "transform 0.2s", "&:hover": { transform: "translateY(-3px)" } }}>
              ...
              <Typography variant="h4" sx={{ fontWeight: 700, color: "var(--forest-2)" }}>
...
const StatCard = React.memo(({ s }) => (
  <Card sx={{
    ...
    <CardContent sx={{ flexGrow: 1, p: 2, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
      ...
      <Typography variant="h4" sx={{ fontWeight: 700, color: "var(--forest-2)", my: 1 }}>{s.value}</Typography>
...
const GuideProgress = React.memo(({ trend }) => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/dashboard/stats?parkId=${selectedPark}`)
      ...
      .then(data => {
        if (isMounted) {
          setGuideStats([
            { label: "Total Guides", value: data.totalGuides, total: data.totalGuides, color: "#874500", icon: <People /> },
            { label: "Certified", value: data.certified, total: data.totalGuides, color: "#6bdc45", icon: <CheckCircle /> },
            { label: "In Training", value: data.inTraining, total: data.totalGuides, color: "#ff7a1a", icon: <School /> },
          ]);
        }
      })
      .catch(() => {});
  ...
  return (
    <Box sx={{ mt: 4 }}>
      <Typography variant="h5" sx={{ mb: 3, fontWeight: 700, color: "var(--forest)", pb: 1, borderBottom: "3px solid var(--citrus)", width: "fit-content" }}>
          ...
          {guideStats.map((s, i) => (
            <Card key={i} sx={{ borderRadius: 3, boxShadow: "0 2px 10px rgba(0,0,0,0.05)" }}>
              <CardContent sx={{ p: 2 }}>
                ...
                <Typography variant="h5" sx={{ fontWeight: 700, color: "var(--forest)", mb: 1 }}>
              ...
              <Box sx={{ flexGrow: 1, width: "100%" }}>
                <ResponsiveContainer width="100%" height={350}>
                  <LineChart data={trend} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Line type="monotone" dataKey="completed" stroke="#6bdc45" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                    <Line type="monotone" dataKey="certified" stroke="#ff7a1a" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              </Box>
            </CardContent>
...
function AdminPage() {
  return (
    <Admin theme={sfcAdminTheme} dataProvider={dataProvider} dashboard={Dashboard} layout={MyLayout}>
      ...
      <Resource name="enrollments" list={CourseRequestsPage} />
      <CustomRoutes>
        <Route path="/ranger" element={<ParkRangerConsole />} />
        <Route path="/training" element={<Navigate to="/admin/course" replace />} />
      </CustomRoutes>
    </Admin>
  );
}
