...
const StudentManagement = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:4001/api/parks")
      ...
      .then((data) => {
        const parkList = Array.isArray(data) ? data : Array.isArray(data?.parks) ? data.parks : [];
        setParks(parkList);
      })
      .catch((err) => {
  ...
  const getAvatarColor = (name) => {
    if (!name) return '#874500';
    const colors = ['#874500', '#6bdc45', '#ff7a1a', '#ffd23f', '#2f6f26', '#b8f22f', '#603100'];
    let hash = 0;
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 4 }, bgcolor: "background.default", minHeight: "100vh" }}>
      <Paper elevation={0} sx={{ p: 3, mb: 3, borderRadius: 3, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 2, bgcolor: "background.paper" }}>
        <Box>
          <Typography variant="h4" sx={{ fontWeight: 800, color: "text.primary", display: "flex", alignItems: "center", gap: 1 }}>
            <PersonIcon color="primary" /> Guide Management
          </Typography>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: 3, display: 'flex', gap: 2, alignItems: 'center', flexWrap: "wrap", bgcolor: "background.paper" }}>
                ...
                {student.progressPercent !== undefined && (
                  <Box sx={{ mb: 2 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                      ...
                      <Typography variant="caption" fontWeight="bold" sx={{ color: "primary.dark" }}>{student.progressPercent}%</Typography>
                    </Box>
