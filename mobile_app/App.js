...
/**Mark Read*/
  ...
  const handleResourceDownload = async (resource) => {
    ...
    const url = resource.url
      ...
      : api.getModuleResourceDownloadUrl(resourceCourseId, resourceModuleId, resource.resource_id, sessionUser.user_id)
    try {
          ...
          <View style={styles.stack}>
            ...
            <View style={styles.card}>
              ...
              {resourcesLoading && resources.length === 0 ? (
                ...
                resources.map((res) => (
                  <View key={res.resource_id || res.id || res.file_id} style={styles.scheduleCard}>
                    <View style={styles.rowBody}>
                      <Text style={styles.rowTitle} numberOfLines={2}>{res.original_name || res.name || res.file_name || res.title}</Text>
                      <Text style={styles.rowMeta}>
                        {formatBytes(res.size_bytes ?? res.sizeBytes)} • {selectedResourceCourse?.course_name || res.course_name || resourceCourseId}
                      </Text>
                      <Text style={styles.rowMeta}>
                        Uploaded {formatUploadDate(res.created_at || res.uploaded_at || res.uploadedAt)}{res.uploaded_by_name ? ` by ${res.uploaded_by_name}` : ''}
                      </Text>
                    </View>
                    <View style={styles.scheduleItemActions}>
