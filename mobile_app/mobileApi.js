...
const contentBaseFromAuthBase = (authBase) => {
  // Auth server usually runs on :4000 while the database-backed user API is :4001.
...
const buildCertificateSummaries = ({ courses, modules, certifications }) =>
  courses.map((course) => {
    const courseModules = modules.filter((module) => String(module.course_id) === String(course.course_id));
    const doneModules = courseModules.filter((module) => Number(module.progress) >= 100).length;
    ...
    return {
      ...
      all_modules_done: courseModules.length > 0 && doneModules === courseModules.length,
...
export const mobileContentApi = (authBaseUrl) => {
  ...
  const loadCoursesCatalog = async (userId) => {
    ...
    try {
      const res = await fetch(withUserId(`${base}/api/courses`, userId));
    ...
    try {
      const res = await fetch(withUserId(`${adminBase}/api/courses`, userId));
      adminData = await toJson(res);
      adminCourses = Array.isArray(adminData.courses) ? adminData.courses.map(normalizeCourse) : [];
    } catch (error) {
      adminError = error;
    }
  ...
  return {
    ...
    getModules: async (userId) => {
      const res = await fetch(withUserId(`${base}/api/training-modules`, userId));
    ...
    getCourses: async (userId) => {
      ...
      if (courses.length === 0) {
        const moduleData = await fetch(withUserId(`${base}/api/training-modules`, userId)).then(toJson);
    ...
    registerCourse: async (courseId, userId) => {
      const res = await fetch(withUserId(`${base}/api/enrollments/requests`, userId), {
        method: "POST",
    ...
    saveProgress: async (moduleId, payload) => {
      ...
      const res = await fetch(withUserId(`${base}/api/progress/${encodeURIComponent(moduleId)}`, userId), {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...payload,
    ...
    getCertificates: async (userId) => {
      const [certRes, courseCatalog, moduleRes] = await Promise.all([
        fetch(withUserId(`${base}/api/certifications`, userId)).then(toJson),
        loadCoursesCatalog(userId),
        fetch(withUserId(`${base}/api/training-modules`, userId)).then(toJson),
      ]);
      ...
      return { certificates: buildCertificateSummaries({ courses, modules, certifications }) };
    },
    requestCertificate: async (userId, courseId) => {
      const res = await fetch(withUserId(`${base}/api/certifications/request`, userId), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId, user_id: userId, courseId, course_id: courseId }),
      });
    ...
    getNotifications: async (userId) => {
      const res = await fetch(withUserId(`${base}/api/notifications`, userId));
      const data = await toJson(res);
      return {
        ...data,
    ...
    markNotificationRead: async (id, userId, read = true) => {
      const res = await fetch(withUserId(`${base}/api/notifications/${encodeURIComponent(id)}/read`, userId), {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    markAllNotificationsRead: async (userId) => {
      const res = await fetch(withUserId(`${base}/api/notifications/read-all`, userId), {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    getSchedule: async (userId) => {
      const res = await fetch(withUserId(`${base}/api/schedule`, userId));
      const data = await toJson(res);
    ...
    updateSchedule: async (id, payload) => {
      const userId = firstValue(payload?.userId, payload?.user_id);
      const res = await fetch(withUserId(`${base}/api/schedule/${encodeURIComponent(id)}`, userId), {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...payload, user_id: userId }),
      });
    ...
    getProfile: async (userId) => {
      const res = await fetch(withUserId(`${base}/api/user-profile`, userId));
      const data = await toJson(res);
    ...
    updateProfile: async (userId, payload) => {
      const updates = [
        ...
        ["assignedPark", payload?.assignedPark],
      ...
      for (const [field, value] of updates) {
        const res = await fetch(withUserId(`${base}/api/user-profile`, userId), {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId, user_id: userId, field, value }),
        });
        await toJson(res);
      }

      return { ok: true, profile: normalizeProfile(payload) };
    },
    getModuleResources: async (courseId, moduleId, userId) => {
      const res = await fetch(withUserId(`${base}/api/course-files`, userId));
      const data = await toJson(res);
